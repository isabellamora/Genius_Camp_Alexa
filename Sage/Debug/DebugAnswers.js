...
const handlers = {
    'LaunchRequest': function () {
      ...
      "' Celine's favorite blank is blank' " + "First Question: " + "What is Celine's favorite color? ";
      this.response.speak(speechOutput).listen("What would you like to do?");
      this.emit(':responseReady');
    },
    'FavoriteColorIntent': function () {
        var colorAnswer = "yellow"; 
        var userGuess = this.event.request.intent.slots.myColor.value;
        var speechOutput; 
        if(colorAnswer == userGuess){
            speechOutput = "You are correct!";
        }
        else{
            speechOutput = "Sorry, better luck next time!"; 
        }
    ...
    'FavoriteSportIntent': function () {
        var speechOutput; 
        var sportAnswer = "basketball";
        var userGuess = this.event.request.intent.slots.mySport.value; 
        if(sportAnswer == userGuess){
            speechOutput = "Nice! You really know her!";
        }
        else{
            speechOutput = "Yikes, maybe you should pay more attention"; 
        }
    ...
    'FavoriteSnackIntent': function () {
        ...
        var userGuess = this.event.request.intent.slots.mySnack.value; 
        var speechOutput; 
